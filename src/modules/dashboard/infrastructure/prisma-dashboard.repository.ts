...
export class PrismaDashboardRepository implements DashboardRepository {
  async getSummary(): Promise<DashboardSummary> {
    ...
    const [totalProducts, inStock, noStock, todayEntries, todayExits, lowStockCount, totalStockValue] = await Promise.all([
      ...
      `,
      prisma.$queryRaw<TotalRow[]>`SELECT COALESCE(SUM(current_stock * cost_price), 0) as total FROM products WHERE deleted_at IS NULL AND active = true`,
    ]);
  ...
  async getMovementsChart(days: number): Promise<MovementChart> {
    ...
    const mapChart = (rows: Array<{ date: string; total: bigint }>): ChartEntry[] =>
      rows.map((e) => ({ date: e.date, total: Number(e.total) }));

    return {
      entries: mapChart(entries),
      exits: mapChart(exits),
    };
  }
  ...
  async getTopProducts(limit = 10): Promise<TopProduct[]> {
    ...
    return rows.map((p) => ({
      ...
      imageUrl: p.imageUrl,
      movement_count: Number(p.movement_count),
      total_entries: Number(p.total_entries),
      total_exits: Number(p.total_exits),
    }));
  }

  async getExpiringSoon(days: number): Promise<ExpiringProduct[]> {
